...
Applying the vocabulary changes how we tokenize. Once the accepted bi-grams are known, we make a second pass over each document, and over every query the same way. At each position we emit the single token, and if the current token together with the next one forms an accepted bi-gram, we emit the phrase token as well, then advance by one position so overlapping phrases are still caught. Keeping the single tokens alongside the phrase is deliberate, as the chapter opener showed: a query for "Holmes" must still match a document that only contains "Sherlock Holmes".
