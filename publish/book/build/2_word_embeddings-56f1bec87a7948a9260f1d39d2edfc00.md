...
The Skip-Gram model operates within a context window of size $2m+1$ around a center word. It trains the model to predict which words typically appear nearby a given center word. As a consequence, words that share similar neighbors are pushed toward similar vectors during training.

For the phrase "the dog chases a cat" with center word "chases" and $m=2$, Skip-Gram models the joint probability of the surrounding words as a product of independent predictions (see [](#fig-skipgram-context)):
