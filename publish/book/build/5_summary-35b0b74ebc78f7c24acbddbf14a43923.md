...
$$\text{sim}_{\text{BM25}}(Q, D) = \sum_{t \in Q} \log \frac{N - \text{df}(t) + 0.5}{\text{df}(t) + 0.5} \cdot \frac{\text{tf}(t, D) \cdot (k + 1)}{\text{tf}(t, D) + k \cdot \left(1 - b + b \cdot \frac{|D|}{adl}\right)}$$

Saturating tf with document-length normalization. Parameters: $k \in [1, 2]$ controls tf saturation, $b \in [0, 1]$ controls length penalty, $adl$ is average document length.
```
