...
```{admonition} Accuracy hides per-class failures
:class: warning

Accuracy (= micro-average) is the standard headline metric for classification, but it is dominated by the largest classes. In our example, `knowledge_base` contributes 70 of the 153 correct predictions while `clarify` contributes only 5. A system that always predicts `knowledge_base` would reach 80/200 = 40% accuracy on that class alone. When class sizes differ substantially and every class matters (as in intent routing, where a misrouted `clarify` query produces a wrong answer), supplement accuracy with macro-averaged precision and recall to expose failures on rare classes.
```
