...
```{note} Key Formula: Skip-Gram Loss
...
```{note} Key Formula: GloVe Cost Function
...
**Average pooling** computes the element-wise mean across all token embeddings. Given $n$ token embeddings $\mathbf{x}_1, \ldots, \mathbf{x}_n$, each in $\mathbb{R}^d$:

$$
\mathbf{e}_{\text{avg}} = \frac{1}{n} \sum_{i=1}^{n} \mathbf{x}_i
$$
...
$$
e_{\text{max},j} = \max_{1 \leq i \leq n} x_{i,j} \quad \text{for each dimension } j = 1, \ldots, d
$$
