...
The intuition behind **pointwise mutual information** is straightforward. Two words that mean something together should co-occur more often than their individual frequencies would suggest under independence. In *A Study in Scarlet*, a corpus of about 43,900 tokens, "said" appears 207 times and "Holmes" 98 times. If the two were independent, we would expect the bi-gram "said Holmes" to occur about $207 \cdot 98 / N \approx 0.5$ times. It actually occurs 12 times, roughly 25 times more than chance. That is a real signal, but a modest one: "said" precedes dozens of different names throughout the book.

Compare "Sherlock Holmes". "Sherlock" appears 52 times, and every one of those 52 occurrences is followed by "Holmes". Independence would predict about $52 \cdot 98 / N \approx 0.1$ co-occurrences; the observed count of 52 is roughly 450 times higher. "Sherlock" is effectively bound to "Holmes".
...
Two things help read the formula. First, $\log_2(N)$ is the same constant for every bi-gram, so ranking by PMI is the same as ranking by $\log_2 \text{tf}(t_1, t_2) - \log_2 \text{tf}(t_1) - \log_2 \text{tf}(t_2)$. Second, look at what maximizes the score. PMI is largest when all three counts equal 1: a word that occurs once, sitting next to another word that occurs once, giving $\text{pmi} = \log_2 N$. Even when the two words and their pair all occur $n$ times together and never apart, the score is $\log_2(N/n) = \log_2 N - \log_2 n$, which only falls as $n$ grows. So PMI rewards rarity, and this is the mirror image of the stop-word problem: instead of frequent function words dominating, two rare words that happen to land side by side by chance float to the top. In *A Study in Scarlet*, 202 different bi-grams reach the maximum score of $\log_2 N \approx 15.4$, every one of them a pair of words that each occur exactly once and only next to each other ("aqua tofana", "assistant surgeon", "arch rebel"). All of them outrank "Sherlock Holmes". A single chance adjacency is no evidence of a real collocation; there is simply too little frequency to be confident the two words prefer each other. The cure is a minimum-frequency filter: require a bi-gram to occur at least, say, three times before scoring it at all, which drops the low-evidence rare pairs.

```{admonition} Example: PMI on the Sherlock Holmes corpus
:class: example

Ranked by PMI with a minimum bi-gram frequency of 3 and a stop-word filter, the top of the list for *A Study in Scarlet* (about 43,900 tokens, 122 bi-grams surviving the filter) looks like this:

| Rank | Bi-gram | tf($t_1$, $t_2$) | tf($t_1$) | tf($t_2$) | PMI |
|---|---|---|---|---|---|
| 1 | audley court | 3 | 3 | 4 | 13.42 |
| 2 | lauriston gardens | 6 | 6 | 6 | 12.84 |
| 3 | salt lake | 10 | 11 | 10 | 11.96 |
| 4 | scotland yard | 6 | 8 | 9 | 11.84 |
| 5 | baker street | 6 | 6 | 29 | 10.56 |
| ... | ... | | | | |
| 47 | sherlock holmes | 52 | 52 | 98 | 8.81 |

Even after the frequency filter, PMI's rare-word bias persists. The top spots go to place names whose parts are individually rare and appear almost only together ("Audley Court", "Lauriston Gardens"). "Sherlock Holmes", the defining phrase of the book, ranks only 47th of 122: because "Holmes" occurs 98 times, the larger denominator drags its PMI down to 8.81. The filter removed the tf=1 noise but did not fix the underlying preference for rarity.
```
...
|---|---|---|---|---|---|
| 1 | sherlock holmes | 52 | 52 | 98 | 669 |
| 2 | jefferson hope | 34 | 37 | 56 | 459 |
| 3 | john ferrier | 29 | 39 | 62 | 353 |
| 4 | brixton road | 13 | 15 | 27 | 188 |
| 5 | salt lake | 10 | 11 | 10 | 181 |
| 8 | scotland yard | 6 | 8 | 9 | 98 |
| 11 | baker street | 6 | 6 | 29 | 89 |

LHR puts the main characters and places of the novel on top: "Sherlock Holmes", "Jefferson Hope", "John Ferrier", "Brixton Road". Without the stop-word filter, grammar pairs such as "of the", "to be", and "had been" crowd into the top of the list alongside these names, because those pairs genuinely are non-independent even though they are useless as phrases. "I am" shows the effect at its most extreme: 39 of the 41 occurrences of "am" follow "I", so the dependence is overwhelming, yet the phrase carries no content. The stop-word filter removes this whole class of pairs.
```

The two measures rank the same corpus differently. PMI favours bi-grams where the components are rare and locked together; LHR favours bi-grams where the components co-occur far more often than chance predicts, whether or not the components are rare. "Sherlock Holmes" makes the contrast concrete: it ranks 47th of 122 under PMI, held down because "Holmes" is common, but first under LHR, which rewards the sheer weight of 52 co-occurrences. In practice both are useful, and many systems compute both and take the union of their top-$k$ lists.
