...
The Skip-Gram model operates within a context window of size $2m+1$ around a center word. It trains the model to predict which words typically appear nearby a given center word. Consider the example phrase "the dog chases a cat" with center word "chases" and $m=2$ (5 words context window in toal). Then, Skip-Gram <models|find alternative word for to model as model is too close to the introductory phrase Skpi Gram model> the probability that the center words contains exactly thes $2m=4$ words in its context window as a product of independent predictions of each word occuring in any context window of that center word(see [](#fig-skipgram-context)):
