...
Before the system can index documents or match user queries, it must turn raw text into a structured, searchable form. The feature extraction pipeline transforms each document through a sequence of stages: extracting plain text from source formats, tokenizing it into terms, normalizing those terms, and finally producing weighted feature vectors. This results in two key artifacts:
...
Text document indexing pipeline. A sequential pipeline transforming a raw document into an inverted index through the stages of text extraction, tokenization, stemming/normalization, and term-frequency counting, ultimately producing an index and vocabulary suitable for full-text retrieval.
```
...
The first step extracts plain text and metadata from the source format. Documents arrive in many formats: PDF, EPUB, plain text, or HTML. The extraction step strips formatting and control sequences, isolates the character stream, and records metadata attributes (author, title, date) for later use in filtering.

At this point, we must decide on a character encoding for the index. UTF-8 is the dominant standard, though encoding choices can affect support for certain languages and scripts.

```{admonition} Web documents and link structure
:class: seealso
HTML documents carry additional structure: anchor texts, heading hierarchies, and hyperlink networks. These provide valuable signals for retrieval but require specialized treatment. We cover HTML-specific extraction and link-based relevance (including PageRank) in the chapter on Web Retrieval.
```
