...
In practice, BM25 is not replaced by vector search. Instead, production systems combine both in hybrid pipelines whose complexity scales with the collection size:

| Pipeline | Scale | Approach |
|----------|-------|----------|
| A: BM25 only | Any | Fast keyword retrieval via inverted index. Misses semantic matches. |
| B: BM25 + reranker | Small-medium | BM25 retrieves candidates, a cross-encoder reranks top-$k$. Simple, effective. |
| C: Semantic + reranker | Small | Bi-encoder retrieves from vector index, cross-encoder reranks. Good recall, misses exact matches. |
| D: Hybrid + reranker | Medium-large | BM25 and bi-encoder retrieve in parallel, results are fused, then a cross-encoder reranks. Covers both lexical and semantic matches. |
| E: Multi-stage | Very large | Coarse retrieval with small embeddings, refined with full embeddings, then cross-encoder. Optimizes cost across billions of documents. |

```{figure} images/figure_5_1.png
:name: fig-retrieval-pipeline
:width: 80%

Pipeline variants for semantic search at different scales. This chapter covers the models that produce the dense representations (bi-encoders, cross-encoders); the next chapter covers how to search through them efficiently (vector indices, ANN algorithms).
```

This chapter focuses on the models that produce the representations: how to build the bi-encoder that generates document and query embeddings, how the cross-encoder scores relevance, and what tradeoffs determine which pipeline to choose. The indexing and retrieval of dense vectors is the subject of [Chapter 6 (Vector Search)](#).
