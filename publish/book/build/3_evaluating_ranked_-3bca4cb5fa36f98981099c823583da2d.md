...
```{admonition} Example: MRR for fact lookups
:class: example
...
System B wins on MAP despite finding far fewer relevant documents on the largest need. Two factors combine: B has perfect AP on two smaller needs, while A's zero on Russian novels costs 0.25 in the average, and A also fails to find one of the five plays (buried child) in its stage-plays run.
```
