...
The Skip-Gram model operates within a context window of size $2m+1$ around a center word and learns to predict context words from the center word. The core idea: if a word vector is good, it should be able to predict which words typically appear nearby. Words that share similar neighbors will be pushed toward similar vectors during training.
