...
The attention mechanism operates on embedding vectors $\mathbf{x}_1, \ldots, \mathbf{x}_n$, but raw text is a string of characters. Three steps transform text into the input the transformer expects:

1. **Tokenization**: the text is split into sub-word tokens. Unlike word-level tokenization (one token per space-separated word), sub-word tokenization breaks words into smaller units that balance vocabulary size against coverage. Two common algorithms are:

   - **Byte Pair Encoding (BPE)**: starts with individual characters, then iteratively merges the most frequent adjacent pair into a new token. After $k$ merges, the vocabulary contains the original characters plus $k$ learned sub-word units. The word "playing" might become ["play", "ing"] if "play" and "ing" were frequent enough to be merged early.
   - **WordPiece**: similar to BPE but selects merges that maximize the likelihood of the training corpus rather than raw frequency. Used by BERT. A prefix "##" marks continuation tokens: "playing" becomes ["play", "##ing"].

   Both algorithms produce a fixed vocabulary (typically 30k-50k tokens) that can represent any input text, including rare words and neologisms, by decomposing them into known sub-word pieces. This connects directly to fastText's sub-word idea from the previous section, but here the sub-word units are determined by a data-driven algorithm rather than fixed character n-grams.

2. **Token embedding lookup**: each token ID is mapped to a $d$-dimensional vector via a learned embedding matrix, exactly like the word embeddings from the previous section. The difference is that these embeddings are trained jointly with the transformer layers, not separately.

3. **Positional encoding**: self-attention is permutation-invariant. The formula $\mathbf{z}_i = \sum_j \alpha_{ij} \mathbf{v}_j$ depends on the content of tokens (via Q, K, V) but not on their order. Rearranging the input tokens would produce the same attention weights. To preserve sequence order, a positional encoding is added to each token embedding before the first attention layer. This can be a fixed sinusoidal function (original transformer) or a learned vector per position (BERT).

The result is a sequence of $d$-dimensional vectors that encode both the identity and position of each token. These serve as the $\mathbf{x}_i$ inputs to the first self-attention layer.
