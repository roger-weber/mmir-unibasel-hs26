...
```{important} Key Formula: Naive Bayes Decision Rule
...
````{note} Example: Detecting German for "Bücher von Goethe"
...
```{warning} Naive Bayes prefers uniform priors for query classification
...
```{hint} Hands-on: Intent Routing
Train a Naive Bayes language detector on a small multi-language corpus, then extend it into an intent classifier over a set of example queries, and walk a query through the full routing pipeline.

[Open notebook ->](https://github.com/roger-weber/mmir-unibasel-hs26/blob/main/demos/ch03-05-intent-routing.ipynb)
