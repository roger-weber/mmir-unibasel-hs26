...
3. **Narrow context window**: Word2Vec operates within a window of 5-10 words, strictly within sentence boundaries. It cannot connect information across sentences. Consider: "The car arrived. It was red and made a lot of noise." In the second sentence, "red" and "noise" refer to "car", but a model limited to the second sentence alone cannot make this connection. The pronoun "it" receives a generic vector unrelated to "car".
...
The problem is scale. A vocabulary of 500,000 words requires 500,000-dimensional input vectors, most of which are zeros. Each neuron in the first hidden layer has 500,000 weights. Expanding the vocabulary means adding dimensions and retraining the entire model.

## Sub-Word Tokenization

Sub-word tokenization addresses both the vocabulary size and the OOV (out-of-vocabulary) problems. Instead of one token per word, text is split into smaller pieces from a compact, fixed vocabulary (typically 30k-50k tokens). Any word, including one never seen during training, can be decomposed into known sub-word pieces. This connects to fastText's sub-word idea, but the sub-word units are determined by a data-driven algorithm rather than fixed character n-grams.
