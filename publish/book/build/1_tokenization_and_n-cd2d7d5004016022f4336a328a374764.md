...
```
['I', 'buy', 'my', 'parents', 's', '10', 'of', 'U', 'K', 'startup',
...
- **Possessives collapse to a stray `s`**. `parents'` and `Watson's` both yield a lone `s`. For retrieval, folding "Watson's" into "Watson" is often desirable (users search for the name, not the possessive), so we typically discard the stray `s`. For syntactic analysis this is harmful, because the link between "Watson" and "cat" is broken.
...
Both `nltk` and `spaCy` ship word tokenizers trained on text corpora that handle the four problems above. On the same sentence they produce nearly identical output:

```python
import nltk
nltk.word_tokenize("Dr. Watson's cat called Mrs. Hersley")
# ['Dr.', 'Watson', "'s", 'cat', 'called', 'Mrs.', 'Hersley']

import spacy
nlp = spacy.load('en_core_web_sm')
[t.text for t in nlp("Dr. Watson's cat called Mrs. Hersley")]
# ['Dr.', 'Watson', "'s", 'cat', 'called', 'Mrs.', 'Hersley']
```

Applied to the demonstration sentence, the improvements over the naive regex are:

- Possessives become their own token (`"'s"`), so `Watson` stays intact while the possessive is still available for filtering.
- Numbers, percentages, and currency amounts stay together when the tokenizer recognizes the shape (`1.4`, `10%`, `$1.4`).
- Common abbreviations are learned from data, so `Dr.`, `Mrs.`, and `U.K.` survive with their trailing periods. The artificial `w.r.o.n.g.` breaks in spaCy's neural tokenizer because it has never seen a five-letter abbreviation of that form.
- Punctuation is preserved as separate tokens, ready to be filtered out downstream if the retrieval pipeline does not want it.
