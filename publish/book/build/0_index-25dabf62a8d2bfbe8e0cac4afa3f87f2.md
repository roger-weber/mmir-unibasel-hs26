...
A different user asks "Bücher von Goethe" and gets a page of unrelated web results. The system saw three tokens and matched none of them well. Nothing in the pipeline noticed that "Bücher" is German for "books", that "Goethe" is a person's name, or that the whole sentence is a request for works by an author. All the information the user gave was there in plain text and none of it was used.

Both failures come from the same source. Classical retrieval treats text as a bag of exact tokens: one surface form is one term and a query is a bag of query terms. That model works for keyword searches over clean text and breaks in two directions. On the document side, one concept has many surface forms and recall collapses. On the query side, natural-language input has structure, meaning, and intent, and the retriever ignores all three.
