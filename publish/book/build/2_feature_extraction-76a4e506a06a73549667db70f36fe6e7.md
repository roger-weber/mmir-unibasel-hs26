...
section: From Text to Searchable Vectors
order: "1.2"
---
...
```{note} Source formats and extraction tools (optional reading)
:class: dropdown
...
```{seealso} Web documents and link structure
HTML documents carry additional structure beyond text: anchor texts, heading hierarchies, and hyperlink networks. These provide valuable signals for retrieval but require specialized treatment. We cover HTML-specific extraction and link-based relevance (including PageRank) in the chapter on Web Search.
```
...
```{seealso} Advanced chunking strategies for RAG
When building systems that feed passages to language models, splitting becomes significantly more nuanced. Overlapping windows, semantic chunking, and hierarchical approaches are covered in the chapter on Retrieval-Augmented Generation.
```
...
```{seealso} Beyond word-level tokens
...
```{seealso} Beyond inflection: compounds, synonyms, and semantic relations
...
The most well-known rule-based stemmer for English is the Porter Algorithm (1980); see [**An Algorithm for Suffix Stripping**](http://www.tartarus.org/martin/PorterStemmer/) in the Further Reading section of the chapter summary. It applies an ordered sequence of suffix-stripping rules to produce a shared pseudo-stem. For the retrieval pipeline, the important result is that related forms map to the same vocabulary term; the exact rule sequence is secondary.
...
```{note}
The internal mechanics of Porter are covered in detail in the chapter on Advanced Text Processing, along with modern alternatives like Snowball and dictionary-based lemmatization.
```
...
Karen Spärck Jones (1972) captured this intuition in a single formula called the **inverse document frequency** (IDF); see [**A Statistical Interpretation of Term Specificity and Its Application in Retrieval**](https://dmice.ohsu.edu/bedricks/courses/cs606-ir/papers/sparck_jones_1972.pdf) in the Further Reading section of the chapter summary. It builds on one statistic: the **document frequency** $\text{df}(t)$, which counts how many documents contain term $t$ at least once. IDF then weights terms inversely to their commonness:

```{important} Key Formula: Inverse Document Frequency
...
| Term | $\text{df}(t)$ | $\text{idf}(t) = \log\frac{10{,}001}{\text{df}+1}$ | Interpretation |
|------|---:|---:|---|
...
A query "retrieval algorithm" would weight "retrieval" roughly 2.5× more than "algorithm" because it appears in far fewer documents. The stop word "the" contributes almost nothing even if present in the query.
```
...
With the vocabulary established and each term assigned an IDF weight, we can now represent documents as vectors. Each document becomes a point in an $M$-dimensional space where $M$ is the vocabulary size and each dimension corresponds to one term. [Figure %s](#fig-bag-of-words-pipeline) shows this process: tokenized text is stemmed, stop words are removed, and the remaining terms are counted to produce per-document frequency vectors.
...
Let $D_{i}$ be the $i$-th document in a collection of $N$ documents. The feature representation $\mathbf{d}_{i}\in \mathbb{R}^{M}$ is a vector where component $d_{i,j}$ describes how term $t_{j}$ occurs in document $D_{i}$. We use $\text{tf}(D_{i},t_{j})$ to denote the number of occurrences of term $t_{j}$ in document $D_{i}$.
...
```{important} Key Formula: **tf·idf** document representation
$$d_{i,j} = \text{tf}(D_{i}, t_{j}) \cdot \text{idf}(t_{j})$$

A term's weight grows with how often it occurs in the document and how rare it is across the collection, so terms that are frequent here but uncommon elsewhere dominate the representation. This **tf·idf** weighting is the standard document representation for classical retrieval models.
```
...
```{hint} Hands-on: Feature Extraction
Build a feature extraction pipeline in Python: tokenize text, apply stemming, compute term frequencies and IDF weights.

[Open notebook ->](https://github.com/roger-weber/mmir-unibasel-hs26/blob/main/demos/ch01-01-feature-extraction.ipynb)
```
