...
Suppose you need to find all books about cats in a library. A simple string search (Ctrl+F for "cat") scans every book title and returns all matches. Even for a modest collection this is impractical, and the result is merely a Boolean filter: it tells you which book titles contain the string "cat", but it cannot tell you which ones are *most relevant* to what you are looking for, nor can it find books that discuss cats without ever using that word.
