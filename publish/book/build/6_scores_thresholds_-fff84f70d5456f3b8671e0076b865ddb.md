...
A binary classifier assigns a score $s(x) \in [0, 1]$ to each item $x$. Items from the positive class tend to receive higher scores; items from the negative class tend to receive lower scores. If the two score distributions were perfectly separated (all positives above some value, all negatives below), any threshold between them would classify perfectly. In practice, the distributions overlap. [Figure %s](#fig-classification-score-distribution) visualizes how that overlap creates unavoidable false positives and false negatives.
...
Let $f_n(x)$ denote the score distribution of the negative class and $f_p(x)$ the score distribution of the positive class. Each classification rate corresponds to an area under one of these curves, partitioned by the threshold $T$. [Figure %s](#fig-threshold-rates) shows the four resulting areas:
...
```{important} Key Formula: Classification Rates as Integrals
...
```{note} Example: ROC curve construction for a spam filter
...
```{note} Example: AUC interpretation
The AUC can be computed from the step-function ROC curve by summing rectangles: each time a negative item is encountered (FPR steps right by $1/N$), the rectangle's height is the current TPR. For the 20-email spam filter in [Figure %s](#fig-roc-construction), the ROC curve hugs the left edge for the first two spam emails (TPR rises to 20% with FPR still at 0%), then begins stepping right. The resulting AUC is approximately 0.68.
...
```{warning} AUC summarizes but does not prescribe
...
```{hint} Hands-on: Thresholds and ROC
