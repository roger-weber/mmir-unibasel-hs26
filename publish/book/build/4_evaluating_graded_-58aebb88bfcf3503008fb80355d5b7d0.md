...
```{important} Key Formula: Cumulative Gain
...
```{note} Example: Cumulative Gain at k = 5
...
```{important} Key Formula: Discounted Cumulative Gain
...
$$DCG_5(B) = 3.000 + 1.893 + 0.500 + 0 + 0.774 \approx 6.1665$$
...
```{note} Example: DCG@10 with binary relevance
...
| $DCG_{10}$ (binary) | 1.000 + 0.631 + 0.356 + 0.333 + 0.301 = 2.6215 | 1.000 + 0.631 + 0.500 + 0.387 + 0.356 + 0.315 = 3.1895 |
...
```{important} Key Formula: Normalized DCG
...
```{note} Example: nDCG@10 for System A and B
...
$$IDCG_{10} \approx 10.3125$$

The actual DCG@10 values from above (with graded relevance):

$$nDCG_{10}(A) = \frac{4.5992}{10.3125} \approx 0.4460 \qquad nDCG_{10}(B) = \frac{7.8253}{10.3125} \approx 0.7588$$
...
```{warning} nDCG and AP answer different questions
...
```{note} Example: Exponential gain variant
...
System B's lead widens under exponential gain. Its three grade-3 books (gain 7 each) earn far more than A's grade-1 and grade-2 books near the top. System A's rank-1 grade-1 book contributes 1 under both gain functions. Its ratio falls because exponential gain makes the three grade-3 books in the ideal ranking much more valuable, while A places only one of them in its top 10 and only at rank 6.
```
...
```{hint} Hands-on: Graded Evaluation
