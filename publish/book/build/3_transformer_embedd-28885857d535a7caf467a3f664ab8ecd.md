...
Self-attention transforms each token embedding $\mathbf{x}_i$ into a new representation $\mathbf{z}_i \in \mathbb{R}^d$ that incorporates information from the entire sequence. The output $\mathbf{z}_i$ has the same dimensionality as the input $\mathbf{x}_i$, but now "bank" at position 2 will have a different $\mathbf{z}_2$ depending on whether the surrounding words are about finance or rivers. The transformation works in three steps:

1. **Project** each token embedding $\mathbf{x}_i$ into three vectors using learned weight matrices $\mathbf{W}_Q$, $\mathbf{W}_K$, $\mathbf{W}_V$. The query $\mathbf{q}_i$ represents "what this token is looking for", the key $\mathbf{k}_i$ represents "what this token offers to others", and the value $\mathbf{v}_i$ represents "the content this token contributes when attended to":
