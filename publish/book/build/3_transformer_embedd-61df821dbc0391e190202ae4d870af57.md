...
2. **Compute attention weights** for token $i$ over all positions $j$. The raw compatibility between query $i$ and key $j$ is their scaled dot product. Normalizing over all positions produces a probability distribution: $\alpha_{ij} \geq 0$ and $\sum_j \alpha_{ij} = 1$. A high $\alpha_{ij}$ means token $i$ attends strongly to token $j$:
