...
Retrieving more documents raises recall, since additional relevant documents may be included, but it also brings in more non-relevant ones, which lowers precision. Conversely, retrieving fewer documents raises precision but risks missing relevant material. Neither number can be improved in isolation without affecting the other, which is why both are always reported together.
```
