...
```{note} From posterior odds to the BIR score (optional reading)
:class: dropdown
...
```{important} Key Formula: BIR Similarity
...
```{note} Example: Feedback and query expansion
...
$$\text{sim}_{\text{BM25}}(Q,D_i)=\sum_{t\in Q ∩ D_i}\text{idf}_{+}(t)\frac{\text{tf}(D_i,t)(k_1+1)}{\text{tf}(D_i,t)+k_1\left(1-b+b\frac{|D_i|}{\text{avgdl}}\right)}$$
...
```{hint} Hands-on: Retrieval Models
Implement Boolean, TF-IDF, and BM25 retrieval on a small collection and compare their rankings.

[Open notebook ->](https://github.com/roger-weber/mmir-unibasel-hs26/blob/main/demos/ch01-02-retrieval-models.ipynb)
```