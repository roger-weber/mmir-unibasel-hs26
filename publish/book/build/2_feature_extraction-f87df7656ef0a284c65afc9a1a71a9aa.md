...
In practice, teams rarely build extraction from scratch. Modern extraction pipelines rely on established libraries and frameworks:

- **Apache Tika**: Java-based toolkit that auto-detects file types and extracts text and metadata from over 1,000 formats (PDF, DOCX, EPUB, email archives, images via OCR).
- **Unstructured.io**: Python library designed for LLM/RAG pipelines. Handles PDFs, HTML, images, and office documents with built-in chunking support.
