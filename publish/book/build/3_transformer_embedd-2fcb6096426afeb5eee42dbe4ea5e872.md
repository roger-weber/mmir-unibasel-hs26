...
BERT (2019) uses WordPiece with a 30,000-token vocabulary. GPT models use BPE with 50,000 tokens. The word "playing" becomes ["play", "##ing"] in WordPiece or ["play", "ing"] in BPE. In practice, all modern embedding and language models use some variant of BPE or SentencePiece (which supports both BPE and Unigram sub-word algorithms). WordPiece remains in use only through BERT-based models. The table below shows how tokenization choices vary across representative models:

| Model | Parameters | Tokenizer | Vocab size | Max context |
|---|---|---|---|---|
| all-MiniLM-L6-v2 | 23M | WordPiece (BERT) | 30k | 256 |
| nomic-embed-text-v1.5 | 137M | WordPiece (BERT) | 30k | 8,192 |
| BGE-M3 | 560M | SentencePiece (XLM-R) | 250k | 8,192 |
| E5-Mistral-7B | 7B | BPE (Mistral) | 32k | 4,096 |
| NV-Embed-v2 | 8B | BPE (Mistral) | 32k | 32,768 |
| Qwen3-Embedding-8B | 8B | BPE (Qwen) | ~150k | 32,768 |

Two trends are visible: vocabulary sizes range from 30k to 250k tokens (all manageable for an embedding matrix), and context windows have grown from 256 to 32k tokens as models became larger. The larger context windows reduce the need for document chunking, a topic we return to in the next section.
...
Several providers also offer embedding models as proprietary API services (no model weights available):

| Provider | Model | Dims | Max context | Notes |
|---|---|---|---|---|
| OpenAI | text-embedding-3-large | 3072 (reducible) | 8,192 | BPE tokenizer (`cl100k_base`) |
| OpenAI | text-embedding-3-small | 1536 (reducible) | 8,192 | Same tokenizer, lower cost |
| Voyage AI | voyage-4-large | 1024 (256-2048) | 32,000 | Matryoshka dims, binary quantization |
| Voyage AI | voyage-context-4 | 1024 (256-2048) | 120,000 | Contextualized chunk embeddings |

The open-weight models in the previous table (Qwen3, BGE-M3, Nomic, NV-Embed) can be self-hosted and fine-tuned on domain-specific data. Proprietary API models trade this flexibility for convenience and, in some cases, domain-specific variants (Voyage offers models specialized for legal, financial, and code retrieval). 
