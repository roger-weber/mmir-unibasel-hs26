...
Micro-averaging produces 76.5% for both precision and recall, which is the same as the overall accuracy. This happens because in a single-label setting, every item contributes exactly one TP (if correct) or one FP and one FN (if wrong), so the pooled ratios collapse to "correct decisions divided by total decisions". The result is dominated by the largest class: `knowledge_base` contributes 70 of the 153 correct predictions and 80 of the 200 items. The rare `clarify` class, with only 5 correct out of 200 total, barely influences the sum.
