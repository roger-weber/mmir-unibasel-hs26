...
The threshold $T$ partitions the score axis into two regions: items with $s(x) \geq T$ are predicted positive, items with $s(x) < T$ are predicted negative. The four confusion-matrix outcomes map to areas under the two distribution curves:

- **True Negatives**: negative-class items scoring below $T$ (correctly left alone)
- **False Positives**: negative-class items scoring above $T$ (incorrectly flagged)
- **False Negatives**: positive-class items scoring below $T$ (incorrectly missed)
- **True Positives**: positive-class items scoring above $T$ (correctly detected)

```{figure} images/figure_2_21.png
:name: fig-threshold-rates
:width: 95%

Decomposition of the score distributions into the four classification rates. TNR and FPR sum to 1 (they partition the negative-class curve); FNR and TPR sum to 1 (they partition the positive-class curve).
```

Moving the threshold changes the balance between these regions:
...
Instead of evaluating one threshold, the ROC (Receiver Operating Characteristic) curve evaluates all of them simultaneously. It plots TPR (sensitivity) on the vertical axis against FPR (1 - specificity) on the horizontal axis, one point per threshold.

```{admonition} Key Formula: ROC Curve Construction
:class: important

For each possible threshold $T$ from $+\infty$ (predict nothing positive) to $-\infty$ (predict everything positive), compute:

$$TPR(T) = \frac{TP(T)}{P} \qquad FPR(T) = \frac{FP(T)}{N}$$

Plot the resulting points $(FPR(T),\; TPR(T))$ to obtain the ROC curve.
```

The construction is straightforward: sort all items by score in descending order, then sweep the threshold from the highest score down. At each step, one more item crosses into "predicted positive": if it is truly positive, TPR increases; if truly negative, FPR increases. The curve traces the path from $(0, 0)$ to $(1, 1)$.

```{admonition} Example: ROC curve for a spam filter
:class: example

Ten emails, sorted by spam score (5 spam, 5 legitimate):

| Score | Actual | Threshold here | TP | FP | TPR | FPR |
|-------|--------|---------------|----|----|-----|-----|
| 0.95 | spam | 0.95 | 1 | 0 | 0.20 | 0.00 |
| 0.89 | spam | 0.89 | 2 | 0 | 0.40 | 0.00 |
| 0.82 | legitimate | 0.82 | 2 | 1 | 0.40 | 0.20 |
| 0.74 | spam | 0.74 | 3 | 1 | 0.60 | 0.20 |
| 0.68 | spam | 0.68 | 4 | 1 | 0.80 | 0.20 |
| 0.54 | legitimate | 0.54 | 4 | 2 | 0.80 | 0.40 |
| 0.47 | legitimate | 0.47 | 4 | 3 | 0.80 | 0.60 |
| 0.39 | spam | 0.39 | 5 | 3 | 1.00 | 0.60 |
| 0.21 | legitimate | 0.21 | 5 | 4 | 1.00 | 0.80 |
| 0.08 | legitimate | 0.08 | 5 | 5 | 1.00 | 1.00 |

The ROC curve passes through $(0, 0) \to (0, 0.20) \to (0, 0.40) \to (0.20, 0.40) \to (0.20, 0.60) \to (0.20, 0.80) \to (0.40, 0.80) \to (0.60, 0.80) \to (0.60, 1.00) \to (0.80, 1.00) \to (1.00, 1.00)$.

At threshold 0.68, the system catches 80% of spam ($TPR = 0.80$) while falsely flagging 20% of legitimate emails ($FPR = 0.20$). At threshold 0.39, it catches all spam ($TPR = 1.00$) but falsely flags 60% of legitimate mail.
```
...
From the ROC curve in the earlier example, the area under the step-function curve can be computed by summing rectangles. Each time FPR increases (a negative item is encountered while sweeping the threshold), the rectangle's height is the current TPR:

Steps where FPR increases (negative items encountered at scores 0.82, 0.54, 0.47, 0.21, 0.08):
- At score 0.82: current TPR = 0.40, $\Delta FPR$ = 0.20 → area += 0.40 × 0.20 = 0.080
- At score 0.54: current TPR = 0.80, $\Delta FPR$ = 0.20 → area += 0.80 × 0.20 = 0.160
- At score 0.47: current TPR = 0.80, $\Delta FPR$ = 0.20 → area += 0.80 × 0.20 = 0.160
- At score 0.39 (positive, no $\Delta FPR$)
- At score 0.21: current TPR = 1.00, $\Delta FPR$ = 0.20 → area += 1.00 × 0.20 = 0.200
- At score 0.08: current TPR = 1.00, $\Delta FPR$ = 0.20 → area += 1.00 × 0.20 = 0.200

$$AUC = 0.080 + 0.160 + 0.160 + 0.200 + 0.200 = 0.800$$

The filter has an 80% probability of ranking a spam email higher than a legitimate one. This is reasonable but not excellent; the two emails at scores 0.47 (legitimate) and 0.39 (spam) are in the wrong order.
```
...
**Maximize accuracy.** Compute $(TP + TN) / (P + N)$ at each threshold and pick the maximum. This works when classes are balanced and error costs are symmetric. For the spam example, threshold 0.68 gives $TP = 4$, $TN = 4$, accuracy $= 8/10 = 80\%$.
