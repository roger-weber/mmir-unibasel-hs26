...
For the **vector space model**, the postings store term frequencies alongside the document identifiers. The score is the cosine between the query and document TF-IDF vectors, but because the sum runs only over the query terms (all others contribute zero), only those postings are read:
...
The key observation for efficient evaluation is that the document norm $\lVert \mathbf{d} \rVert$ is known at indexing time. If we divide each TF-IDF value by the norm before storing it, the postings for term $t_j$ hold the pre-normalized weight $\hat{d}_j$ directly. At query time the ranker multiplies $\hat{d}_j$ by the query weight $\hat{q}_j$ and sums, which is a plain dot product with no per-document division. The query norm $\lVert \mathbf{q} \rVert$ is a constant for the whole result list and does not affect ranking, so it can be ignored or applied once at the end. In short, normalizing at index time turns the cosine into a single pass of multiply-and-add over the postings.
