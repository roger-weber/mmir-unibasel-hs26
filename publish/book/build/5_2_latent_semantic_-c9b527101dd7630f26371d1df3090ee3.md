...
Projecting the query "human computer interaction" (only "human" and "computer" are in the vocabulary) into the 2D topic space yields the vector $\bar{\mathbf{q}} = (0.138,\ {-0.028})$.
...
The query vector points toward the c-documents. Using cosine similarity, documents rank as: c5 > c2 > c4 > c3 > c1, with all c-documents retrieved before any m-document. Notably, c3 ("The EPS user interface management system") ranks well despite containing none of the query terms. The SVD has recognized that its terms ("EPS", "user", "interface", "system") co-occur with query-related terms in other documents.
