...
- **"cats"** has a negative $c_j$: it occurs only in non-relevant documents, both of which are technical distractors about pets and animal classification rather than the woodland adventure the user wants. A negative weight can therefore help demote or eliminate documents that share surface vocabulary with the query but belong to the wrong sense.
