...
To predict these probabilities, Skip Gram represents each word $w_i$ as two $d$-dimensional vectors: $\mathbf{v}_i$ when used as a center word, and $\mathbf{u}_i$ when used as a context word. Semantic relationships between two words is modelled with the inner vector product, for instance: $\mathbf{u}_s^\top \mathbf{v}_c)$ between the center word $w_c$ and its surrounding word $v_c$. To obtain a probability distirbution for $P(w_s \mid w_c)$ given center word $w_c$ over all words $w_s$ (this includes the center word), we use softmax to obtain (easy to show that $\sum_{i \in \mathbb{T}}) P(w_i \mid w_c) =1$):
