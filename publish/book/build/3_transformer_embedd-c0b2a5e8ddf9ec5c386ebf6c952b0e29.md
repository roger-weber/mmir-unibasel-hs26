...
In practice, all modern embedding and language models use some variant of BPE or SentencePiece (which supports both BPE and Unigram sub-word algorithms). WordPiece remains in use only through BERT-based models. The table below shows how tokenization choices vary across representative models:
...
The open-weight models in the previous table (Qwen3, BGE-M3, Nomic, NV-Embed) can be self-hosted and fine-tuned on domain-specific data. Proprietary API models trade this flexibility for convenience and, in some cases, domain-specific variants (Voyage offers models specialized for legal, financial, and code retrieval). For this course, we focus on open-weight models where the architecture and training are transparent.
