...
The first probability measures how often term $t_j$ occurs in relevant documents; the second measures how often it occurs in non-relevant documents. Applying the independence assumption and taking the logarithm gives an additive score.
...
Before feedback is available, BIR commonly starts with $r_j=0.5$ and estimates $n_j$ from document frequency. The initial ranking can then collect relevance judgements. If $K$ documents have been judged, $L$ are relevant, $k_j$ judged documents contain $t_j$, and $l_j$ of those are relevant, smoothed estimates are

$$r_j=\frac{l_j+0.5}{L+1}, \qquad n_j=\frac{k_j-l_j+0.5}{K-L+1}.$$

The constants avoid probabilities of exactly 0 or 1. Repeated feedback can refine the weights, although users may be unwilling to judge many results.
