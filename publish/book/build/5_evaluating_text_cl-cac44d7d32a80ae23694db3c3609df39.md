...
The confusion matrix organizes every prediction the system made into a $2 \times 2$ table for binary classification. [Figure %s](#fig-confusion-matrix-binary) shows this arrangement. Each item in the test set falls into exactly one cell, depending on the system's prediction and the true label.
...
```{important} Key Formula: Classification Metrics
...
[Figure %s](#fig-confusion-matrix-metrics) summarizes how the four outcome counts produce the classification metrics and their complementary rates.
...
```{note} Example: Spam filter metrics
...
```{warning} Which error is worse depends on the application
...
```{note} Example: The accuracy paradox
...
```{warning} When to distrust accuracy
...
```{note} Example: Biometric face unlock
...
````{note} Example: Intent routing
An AI assistant classifies each user query into one of four intents: `knowledge_base` (answer from stored documents), `web_search` (look up current information), `calculator` (compute a numerical answer), or `clarify` (ask the user for more detail). Evaluated on 200 queries, the resulting matrix in [Figure %s](#fig-confusion-matrix-multiclass) makes the uneven per-class performance visible:
...
To compute precision, recall, and specificity for one class, collapse the $K \times K$ matrix into a binary view: "belongs to class $C$" vs. "does not belong to class $C$". [Figure %s](#fig-confusion-matrix-one-vs-rest) shows this collapse for two classes from the intent-routing example.

```{figure} images/figure_2_19.png
:name: fig-confusion-matrix-one-vs-rest
:width: 90%
...
$$\text{Recall}_\text{micro} = \frac{\sum_k TP_k}{\sum_k (TP_k + FN_k)} = \frac{70 + 48 + 30 + 5}{80 + 60 + 40 + 20} = \frac{153}{200} = 0.765$$
...
```{warning} Accuracy hides per-class failures
...
```{hint} Hands-on: Classification Evaluation
...
[Open notebook ->](https://github.com/roger-weber/mmir-unibasel-hs26/blob/main/demos/ch02-02-classification-and-roc.ipynb)
