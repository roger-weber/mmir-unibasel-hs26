...
2. **Score** how much token $i$ should attend to each other token $j$. The raw score is the scaled dot product between query $i$ and key $j$. Normalizing over all positions $j$ with softmax produces attention weights that sum to 1:
