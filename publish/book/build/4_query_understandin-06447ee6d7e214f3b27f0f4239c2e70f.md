...
- **Pseudo-relevance feedback**, also called blind feedback or Rocchio-style expansion. Retrieve an initial result set with the user's query, extract the most distinctive terms from the top-$k$ retrieved documents, and add those terms to the query as expansion terms. This is a retrieval-model technique, not a text-processing one, and belongs to the classical text retrieval chapter alongside vector-space and BM25 ranking.
