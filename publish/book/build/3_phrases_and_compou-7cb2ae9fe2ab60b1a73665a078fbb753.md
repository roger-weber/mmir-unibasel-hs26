...
Each probability is estimated as a relative frequency over a corpus of $N$ tokens: $p(t) = \text{tf}(t)/N$ and $p(t_1, t_2) = \text{tf}(t_1, t_2)/N$. Substituting these into the ratio cancels two of the three $1/N$ factors and leaves the single $N$ in the numerator of the right-hand form. PMI is the log-ratio of the observed joint probability to the joint probability that independence would predict. It is high when two tokens almost always appear together and rarely apart.
```
