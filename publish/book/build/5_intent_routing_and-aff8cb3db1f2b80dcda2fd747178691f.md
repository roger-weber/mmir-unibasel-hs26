...
Two comparisons for contrast:

- "Weather in Basel tomorrow?" -> `weather` intent (WH-word implicit, LOCATION entity, DATE entity, question mark, no PERSON or PRODUCT). Route to weather API with city=Basel and date=tomorrow.
- "Who won the F1 race last weekend?" -> `news_search` (WH-word, DATE entity, sports domain terms). Route to news index with date filter set to the past 7 days.
