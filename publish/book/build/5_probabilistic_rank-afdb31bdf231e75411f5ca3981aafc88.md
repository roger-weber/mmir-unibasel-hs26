...
**Initial estimates.** Before any relevance information is available, the system must produce a first ranking. BIR commonly assumes that every query term has an equal chance of occurring in a relevant document, $r_j=0.5$. It estimates occurrence in non-relevant documents from the term's frequency in the complete collection:
