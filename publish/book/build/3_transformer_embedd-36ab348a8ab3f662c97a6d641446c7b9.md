...
In practice, all modern language models use some variant of BPE or SentencePiece (which supports both BPE and Unigram sub-word algorithms). WordPiece remains in use only through BERT-based models. Vocabulary sizes range from 30k to 250k tokens. The exact size is a design choice: larger vocabularies represent more words as single tokens (faster encoding, more parameters in the embedding matrix), while smaller vocabularies decompose more aggressively (fewer parameters, but longer token sequences).
...
Note that the term "embedding" is now overloaded with two meanings: (1) the input embedding layer that maps tokens to vectors as the first step of a neural network, and (2) the semantic embedding of an entire text passage produced by the full model for retrieval. In this subsection, we discuss the input embedding (meaning 1). The retrieval embeddings (meaning 2) are the output of the complete architecture discussed later in this section.
