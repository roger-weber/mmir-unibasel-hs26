...
For the `clarify` class (the rare intent with 20 actual queries), the collapse yields $TP = 5$, $FP = 7$, $FN = 15$, $TN = 173$:
...
For the dominant `knowledge_base` class (80 of 200 queries), the collapse yields $TP = 70$, $FP = 22$, $FN = 10$, $TN = 98$:
