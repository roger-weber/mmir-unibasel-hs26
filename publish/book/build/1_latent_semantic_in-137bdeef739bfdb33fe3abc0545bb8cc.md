...
```{note} LSI in historical context (optional reading)
:class: dropdown
...
[](#fig-svd-truncated) illustrates the truncated decomposition. Compared to [](#fig-svd-full), the matrices are now much smaller: $\mathbf{U}_k$ has only $k$ columns (one per latent topic) and $\mathbf{V}_k^\top$ has only $k$ rows.

```{figure} images/figure_5_4.png
:name: fig-svd-truncated
:width: 90%

Truncated SVD retains only $k$ dimensions. Rows of $\mathbf{U}_k$ are $k$-dimensional term vectors; columns of $\mathbf{V}_k^\top$ (rows of $\mathbf{V}_k$) are $k$-dimensional document vectors.
```
...
When a new document $\mathbf{d}$ arrives (represented as a term vector in the original $m$-dimensional space), we project it into the $k$-dimensional latent space without recomputing the full SVD (see [](#fig-lsi-folding)):
...
Projecting the query "human computer interaction" (only "human" and "computer" are in the vocabulary) into the 2D topic space yields $\bar{\mathbf{q}} = (0.138,\ {-0.028})$ (after normalizing sign conventions), as shown in [](#fig-lsi-cosine-cone):
...
```{note} Full numeric SVD matrices
:class: dropdown
