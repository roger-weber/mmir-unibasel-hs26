...
```{figure} images/svg_attention_mechanism.svg
:name: fig-attention-mechanism
:width: 95%

Self-attention for the token "bank" in "The bank approved the loan". Left: the query q₂ is compared against all keys to produce attention weights; "approved" (0.52) and "loan" (0.36) receive the highest attention, contributing most to the context-aware output z₂. Right: multi-head attention runs h parallel heads in d/h dimensions each, concatenates, and projects back to d dimensions.
```
