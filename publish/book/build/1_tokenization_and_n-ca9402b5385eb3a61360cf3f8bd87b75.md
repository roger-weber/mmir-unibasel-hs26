...
The `pipeline` function in `demos/shared/text.py` composes these steps on top of a regex tokenizer, and the chapter's demo notebook compares the outputs side by side.
...
- **Unicode normalization** applies `unicodedata.normalize('NFKC', text)` to convert combining diacritics and compatibility characters into their canonical form. The character `é` written as `e` plus a combining acute accent ends up identical to `é` written as the single codepoint `\u00E9`.
...
**Spoken-language transcripts** are produced from a phoneme stream, and the segmenter has to decide where word boundaries fall. Non-native pronunciation or unclear articulation shifts those boundaries.
...
- **Stop-word counts** count how many of the top hundred function words of each candidate language appear. English "the", "of", "and"; German "der", "die", "und", "ist"; French "le", "la", "de", "et". Whichever language's stop-word list matches most terms wins.
- **Vocabulary counts** extend the stop-word idea to a longer list of frequent content words.

For short queries of fewer than five or six tokens, rules stop being reliable. `"mein computer"` mixes one German word and one English loanword; `"pain"` is a French word for bread and an English word for suffering. In that regime a statistical classifier over character n-grams performs much better, using the Naive Bayes formulation we return to in [](#advanced-text-processing-intent-routing).

```python
from lingua import Language, LanguageDetectorBuilder

detector = LanguageDetectorBuilder.from_all_languages().build()
detector.detect_language_of("Bücher von Goethe")  # Language.GERMAN

detector.compute_language_confidence_values("Je suis à New York")
# FRENCH: 0.45, ENGLISH: 0.37, ITALIAN: 0.18
```

The `lingua` library reports over 99% accuracy across 70 languages by combining rule-based scoring with character-n-gram Naive Bayes; `langdetect` offers a leaner rule- and n-gram-based detector covering 55 languages. Once the language is known, the pipeline branches: a German document goes to the German Snowball stemmer and the German POS tagger, an English document to the English WordNet lemmatizer, and so on.
