...
Suppose you need to find all documents about cats in a collection of ten million web pages. A simple string search (Ctrl+F for "cat") is far too slow: scanning every character in every document does not scale. Even if it were fast enough, exact string matching is a Boolean filter. It tells you which documents contain the string "cat", but it cannot tell you which ones are *most relevant* to what you are looking for, nor can it find documents that discuss cats without ever using that word.
