...
```{important} Key Formula: Precision and Recall
...
```{note} Example: Precision and Recall for System A and B
...
Fallout is the fraction of non-relevant documents that the system retrieved. For our two systems, $f_A = 13/35 = 37.1\%$ and $f_B = 2/35 = 5.7\%$. Fallout measures the reading cost imposed on someone who inspects every result: the patent lawyer from [Designing a Retrieval Benchmark](#performance-evaluation-benchmark-design) needs high recall, because missing a document is expensive, but also wants low fallout, because every irrelevant document in the list costs time and attention. System A delivers recall of 80% but pulls in more than a third of the irrelevant collection; the ideal for an exhaustive search would push recall toward 1 while keeping fallout near 0.

```{warning} Precision and recall are in tension
...
```{important} Key Formula: F-beta Measure
...
```{note} Example: F-beta for System A and B
...
```{important} Key Formula: Macro Averaging
...
```{warning} Macro averaging is the standard in retrieval evaluation
In retrieval evaluation, the standard practice is macro-averaging: MAP, mean nDCG, and mean MRR all compute a per-topic score and then take the arithmetic mean. Micro-averaging across topics is rarely used, because weighting each document equally would let one large topic dominate all others. The macro/micro choice becomes more relevant in [Evaluating Text Classifiers](#performance-evaluation-text-classifiers), where classes are often imbalanced and the two methods can disagree sharply. When reporting retrieval results, specify both the measure and the number of topics it was averaged over.
```
...
```{hint} Hands-on: Precision and Recall
