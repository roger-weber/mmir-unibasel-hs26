...
```{note} Key Formula: LSI Projection
...
```{note} Key Formula: Skip-Gram Probability
...
```{note} Key Formula: Scaled Dot-Product Attention
...
```{note} Key Formula: ColBERT MaxSim Scoring
...
```{note} Key Formula: Reciprocal Rank Fusion
...
[Take the Chapter 5 Quiz ->](https://roger-weber.github.io/mmir-unibasel-hs26/quiz/)
```
