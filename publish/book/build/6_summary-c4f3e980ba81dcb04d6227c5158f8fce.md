...
7. Phrases and compounds sit on opposite sides of the word-boundary problem. Bi-grams like "New York" pack multiple tokens into one concept and need explicit phrase indexing; compounds like "Bücherregal" hide multiple concepts in one token and can be split into their parts, but only endocentric compounds split safely (a "Bücherregal" is a kind of "Regal"; a "Wolkenkratzer" is not a kind of "Kratzer", so splitting it hurts precision). PMI, LHR, and log-frequency scoring are the classical selectors.
