...
Let $D_{i}$ be the $i$-th document in a collection of $N$ documents. The feature representation $\mathbf{d}_{i}\in \mathbb{R}^{M}$ is a vector where component $d_{i,j}$ corresponds to term $t_{j}$. We use $\text{tf}(D_{i},t_{j})$ to denote the number of occurrences of term $t_{j}$ in document $D_{i}$.
