...
The second option is fast at query time but fragile: if a single document is added or removed, the IDF of every affected term changes, which in turn changes the stored weights and the norms of all documents containing those terms. The entire index would need to be rebuilt. This is one reason why BM25 avoids the cosine measure altogether and uses a plain sum instead. BM25 normalizes by document length through an explicit parameter ($b$ and $\text{avgdl}$) that can be stored cheaply alongside each posting or looked up once per document, without requiring precomputed vector norms or assuming constant IDF values.
