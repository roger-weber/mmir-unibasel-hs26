...
The problem is visible: macro-averaging gives equal weight to every need, so a need with one relevant document pulls the average as hard as a need with fifteen. When a system misses the only relevant item for a rare need, that zero dominates the mean. This behaviour is intentional when every need matters equally, but it can misrepresent overall system capability.
...
```{admonition} Key Formula: Micro Averaging
:class: important
...
Macro-averaging is the right choice when every need matters equally, including rare specialist queries with only one or two relevant documents. A system that ignores any need, however small, is penalised. Micro-averaging is the right choice when overall document-level coverage matters, since it reflects performance on the typical relevant document rather than on the typical need. Report which was used; omitting that information makes the result uninterpretable.
```
```
