...
```{note}
Pure-Python engines such as Whoosh were once popular for learning and prototyping because their internals are easy to read. Whoosh is no longer actively maintained, so we do not build on it here. The concepts it illustrated, a schema of fields, postings with term frequencies and positions, pluggable scoring, are exactly the ones we implement directly and see again below.
```
...
Because a `tsvector` column and a GIN index live inside the database, full-text predicates combine naturally with ordinary SQL: a single query can match text, filter on other columns, join related tables, and run inside a transaction. For collections up to a few million documents on a database you already operate, this is often all the search infrastructure an application needs. Conceptually, one could even build the inverted index by hand as an ordinary table of (term, document, frequency) rows with a B-tree index on the term column; the hands-on notebook does exactly this to make the structure concrete.
...
Apache Lucene, created by Doug Cutting in 1999 and an Apache project since 2001, is the open-source Java library at the core of most production search platforms. It provides the full toolkit of this chapter: analysis, inverted indexing, term weighting, and ranking. [](#fig-lucene-architecture) shows how its pieces fit together. The examples here target the Lucene 9.x API; because major releases change APIs, older online examples may not compile against a current version.
...
Lucene writes the index as a set of immutable **segments**. Each time an `IndexWriter` flushes, it creates a new segment, which reduces contention and protects against corruption. Because segments are immutable, a delete does not remove data; it marks the document in a side file, and an update is a delete followed by an add. A configurable merge policy periodically combines smaller segments into larger ones, and deleted documents are physically dropped only when their segment is merged. Searching runs over all live segments in parallel and merges their results, which is the seed of the distributed scaling in the next section.
