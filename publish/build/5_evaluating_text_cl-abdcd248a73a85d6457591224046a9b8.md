...
Retrieval systems answer queries: given an information need, they decide which documents are relevant and rank them. Classification systems assign labels: given an item, they decide which category it belongs to. An email arrives and the system labels it "spam" or "not spam". An image arrives and the system labels it "cat", "dog", or "bird". A user utterance arrives and an AI assistant routes it to "knowledge base", "web search", "calculator", or "clarify".

The underlying evaluation question is the same: how often does the system make the right decision? Precision and recall still apply. What changes is the framing. There is no "retrieved set" to measure against a collection; instead, the system assigns exactly one label to each item in a test set, and we compare those predictions against ground-truth labels. Three properties make classification evaluation distinct from retrieval evaluation:
...
The binary confusion matrix. Green cells are correct predictions; orange cells are errors.
```
