...
Example: Consider three additional needs alongside the CS-foundations query, answered by the same two systems:
...
**Macro-averaging** does exactly that arithmetic mean: compute $p_i$ and $r_i$ per need, then average.

$$p_{\text{macro}} = \frac{1}{N}\sum_{i=1}^{N} p_i \qquad r_{\text{macro}} = \frac{1}{N}\sum_{i=1}^{N} r_i$$

System A misses the single relevant Russian novel entirely, so that need contributes $p = 0$ and $r = 0$ to the average. A single failure on a rare need pulls the macro average down sharply, even if the system performs well elsewhere. That is the design choice: every need counts equally, regardless of how many relevant documents it has.

The alternative asks a different question. Instead of averaging per-need ratios, it pools all the outcome counts first and computes precision and recall on the combined totals.

**Micro-averaging** sums the true positives, false positives, and false negatives across all needs before dividing:

$$p_{\text{micro}} = \frac{\sum_{i=1}^{N} TP_i}{\sum_{i=1}^{N} (TP_i + FP_i)} \qquad r_{\text{micro}} = \frac{\sum_{i=1}^{N} TP_i}{\sum_{i=1}^{N} (TP_i + FN_i)}$$

Now the Russian-novels failure adds one false negative to a pool that already contains dozens. The large CS-foundations need, where System A finds 12 of 15, dominates the sum simply because it contributes more documents.
...
Macro-averaging gives every need equal weight. Micro-averaging gives every document equal weight, so needs with more relevant documents contribute more. Recall follows the same pattern, replacing $FP_i$ with $FN_i$.
```
...
$$p_{\text{macro},A} = \frac{0.480 + 0.667 + 0.500 + 0.000}{4} = 41.2\%$$

$$p_{\text{macro},B} = \frac{0.750 + 0.750 + 1.000 + 1.000}{4} = 87.5\%$$

System B wins comfortably: it has strong precision on every need, including the one System A missed completely.

**Micro precision:**

$$p_{\text{micro},A} = \frac{12+4+2+0}{25+6+4+3} = \frac{18}{38} = 47.4\%$$

$$p_{\text{micro},B} = \frac{6+3+2+1}{8+4+2+1} = \frac{12}{15} = 80.0\%$$

System B still wins on precision. But now consider recall:

**Macro recall:** B wins, 75.0% against 65.0%, because the Russian-novels miss weighs as heavily as the CS-foundations need.

**Micro recall:** A wins, 78.3% against 52.2%, because pooling the counts lets the large CS-foundations need, where A finds 12 of 15, dominate the sum.

Same systems, same needs, opposite conclusions on recall.
```

The disagreement is not a bug in one method. It reflects a genuine choice about what matters: whether each need counts equally, or whether each document counts equally. Choosing without stating which was used leaves the reader unable to tell whether rare needs were protected or diluted.

```{admonition} Choosing between macro and micro
:class: warning

Macro-averaging is the right choice when every need matters equally, including rare specialist queries that have only one or two relevant documents. Micro-averaging is the right choice when overall document-level effectiveness matters, since it reflects how a typical user with a larger, more common need is served. Report which was used.
```
