...
The prior $P(C_k)$ captures the base rate of each class; the likelihood $P(x_j \mid C_k)$ captures how well feature $x_j$ fits class $C_k$. The evidence $P(\mathbf{x})$ is constant across classes and drops out of the argmax.
```
...
The independence assumption is not literally true (tokens in real text are not independent), but Naive Bayes still performs well on text because token co-occurrences that violate independence are usually captured by the priors. Its speed and small memory footprint are the reasons it stays in production stacks two decades after neural classifiers became viable.
...
- **Features**: character n-grams from a vocabulary chosen per language during training (typically the most frequent 10,000 to 50,000 n-grams).
...
Extract the character trigrams (with `_` marking word boundaries):
...
- `_go`, `goe`, `oet` appear in German names (Goethe, Goebbels, Görlitz) at rates much higher than in English.

The `lingua` library returns `Language.GERMAN` with a confidence above 0.95.
```
...
Every gap named in the chapter opener is closed by a specific step above. The missing "who" is absorbed into the classification features rather than searched as a keyword. The missing "weekend" becomes a temporal range that a race-results page dated Sunday 8 August will match. The missing "race" is replaced by the domain-synonym expansion. What arrives at the news backend is a bag of tokens that actually appear in the intended answer document, restricted to the right week and ranked by date.

The classical pipeline handles this end to end. The remaining edge cases (a race delayed to Monday by weather, a driver name the classifier has never seen, a query in mixed English and Italian) are where dense retrieval and LLM-assisted reformulation start to pay for themselves. Those are the subject of later chapters.

The router does not do the retrieval itself. It picks a backend and passes the structured query to it. Everything upstream in this chapter was building the structure that the router now dispatches on.
