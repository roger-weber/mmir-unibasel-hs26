...
```{admonition} Key Formula: Zipf's Law
:class: important
...
The frequency of a term is inversely proportional to its rank. A few terms dominate the collection; the vast majority are rare.
```
...
This **tf·idf** weighting is the standard document representation for classical retrieval models. The retrieval models in the next sections build directly on these weighted vectors.
