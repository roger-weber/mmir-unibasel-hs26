...
- **Features**: character n-grams from a small per-language vocabulary chosen during training. A few hundred to a few thousand of the most frequent n-grams per language carry almost all of the discriminating signal, so the per-language profiles stay small and the whole detector remains lightweight.
