...
The most well-known rule-based stemmer for English is the Porter Algorithm (1980). It applies an ordered sequence of suffix-stripping rules to produce a shared pseudo-stem. For the retrieval pipeline, the important result is that related forms can map to the same vocabulary term; the exact rule sequence is secondary.
...
Karen Spärck Jones (1972) captured this intuition in a single formula called the **inverse document frequency** (IDF). See **Spärck Jones (1972)**, [A Statistical Interpretation of Term Specificity and Its Application in Retrieval](https://dmice.ohsu.edu/bedricks/courses/cs606-ir/papers/sparck_jones_1972.pdf), in the Further Reading of the chapter summary. It builds on one statistic: the **document frequency** $\text{df}(t)$, which counts how many documents contain term $t$ at least once. IDF then weights terms inversely to their commonness:
