...
$$\text{score}(D, Q) = \sum_{i} \text{score}_i, \quad \text{score}_i = \text{boost}_i \cdot \text{idf}_i \cdot \frac{\text{tf}_i\cdot (k_1 + 1)}{\text{tf}_i + k_1\cdot \left(1 - b + b\,\dfrac{dl}{avgdl}\right)}, \quad \text{idf}_i = \log\left(1 + \frac{N - n_i + 0.5}{n_i + 0.5}\right).$$
