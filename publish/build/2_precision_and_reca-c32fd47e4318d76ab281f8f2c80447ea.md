...
A true positive is a relevant book the system retrieved: a hit. A false positive is a book the system retrieved that turns out not to be relevant: noise in the result. A false negative is a relevant book the system failed to retrieve: a miss. A true negative is a book the system correctly left out. [Figure %s](#fig-retrieval-confusion-sets) shows the same partition as overlapping sets: the collection as a whole, the relevant books inside it, the retrieved books inside it, and their intersection.

```{figure} images/figure_2_3.png
:name: fig-retrieval-confusion-sets
:width: 60%

The document collection split into retrieved and relevant sets. Their intersection is the true positives.
```
...
Precision asks how much of what came back is worth reading. Recall asks how much of what exists was found.
