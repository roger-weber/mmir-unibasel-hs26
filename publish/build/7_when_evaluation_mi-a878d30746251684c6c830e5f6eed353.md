...
[Figure %s](#fig-pooling-sparse) shows the sparse case, typical of benchmarks with thousands of needs. A large part of the genuinely relevant material was never assessed. A system that finds relevant documents outside the pool earns nothing for them, and it can therefore score below a system that finds fewer relevant documents but stays inside the judged set. The bias runs against exactly the systems worth discovering: those retrieving material the existing approaches miss.
...
Recall suffers most. Precision at rank 10 needs only the top ten documents judged, which pooling usually delivers. Recall needs the total amount of relevant material in the collection, and that total is precisely what incomplete judging leaves unknown. A recall figure computed against sparse judgments is a lower bound presented as a measurement.
...
A result that survives these questions is worth acting on. The next section collects the measures themselves, along with guidance on which one fits which task.
