...
For the **vector space model**, the postings store term frequencies alongside the document identifiers. The score is the cosine between the query and document TF-IDF vectors. Because the sum runs only over the query terms (all others contribute zero), only those postings are read:
...
Without any optimization, the ranker would need each document's vector norm $\lVert \mathbf{d} \rVert$ at query time, which means either storing it separately and doing a random lookup per candidate, or recomputing it on the fly. The standard trick is to normalize by document length at index time: rather than storing the raw term frequency, the postings for term $t_j$ hold $\text{tf}(D, t_j) / \lVert \mathbf{d} \rVert$ for each document $D$. This value is stable because it depends only on the document content, not on the collection. At query time the ranker multiplies each stored value by $\text{idf}(t_j)^2 \cdot \text{tf}(Q, t_j) / \lVert \mathbf{q} \rVert$ and sums, giving the cosine without any per-document lookup. The IDF factor is applied at query time because document frequencies change as documents are added; baking IDF into the stored value would force a full index rebuild whenever the collection grows.
