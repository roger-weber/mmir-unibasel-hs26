...
This decision about retrieval granularity defines the collection structure. Finer granularity gives more specific results but increases the number of entries the system must manage. Coarser granularity reduces index size but forces users to locate relevant passages themselves. [Figure %s](#fig-document-chunking-split) illustrates the splitting operation.
...
When a researcher types "neural plasticity AND memory consolidation" into the library search, the online phase begins. The system first tokenizes the query and generates feature vectors using the same pipeline that processed documents during indexing. This symmetry is essential: queries and documents must live in the same feature space for comparison to work.
...
The query's feature representation is then compared against the representations stored in the index. Documents with similar representations are treated as potential matches. Finally, a ranking step scores each candidate and sorts the result list so that the most relevant articles appear first. [Figure %s](#fig-online-query-pipeline) shows the complete online flow.

```{figure} images/figure_1_5.png
:name: fig-online-query-pipeline
:width: 60%

Online query pipeline: tokenize, expand, match against index, rank results.
```
