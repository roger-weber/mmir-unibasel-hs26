...
Karen Spärck Jones (1972) captured this intuition in a single formula called the **inverse document frequency** (IDF). It builds on one statistic: the **document frequency** $\text{df}(t)$, which counts how many documents contain term $t$ at least once. IDF then weights terms inversely to their commonness:
