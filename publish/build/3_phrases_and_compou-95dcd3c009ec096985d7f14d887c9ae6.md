...
The intuition: the dictionary filter first removes any split containing a piece that is not a real word, so ("Wol", "Kenkratzer") is discarded outright because neither piece is a German word. The score then ranks the splits that survive, preferring the one whose parts are the more common words, which is the most probable decomposition: ("Wolke", "Kratzer") wins because both are frequent German nouns. The average is used rather than the sum so that splits with different numbers of parts are comparable.
